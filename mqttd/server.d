...
struct MqttServer(C) if(isMqttConnection!C) {
    ...
    void send(R)(ref C connection, R bytes) if(isInputRangeOf!(R, ubyte)) {
        ...
        switch(type) with(MqttType) {
            ...
            case CONNECT:
                ...
                MqttConnack(code).cerealise!(b => connection.send(b));
                break;
            ...
            case SUBSCRIBE:
                ...
                MqttSuback(msg.msgId, qos).cerealise!(b => connection.send(b));
                break;

            case UNSUBSCRIBE:
                auto msg = decerealise!MqttUnsubscribe(bytes);
                _broker.unsubscribe(connection, msg.topics);
                MqttUnsuback(msg.msgId).cerealise!(b => connection.send(b));
                break;
            ...
            case PINGREQ:
                MqttFixedHeader(MqttType.PINGRESP).cerealise!(b => connection.send(b));
                break;
